...
class SearchService {
  ...
  private def scheduleSearches(): Unit = {
    ...
    searchTaskCreator.scheduler.scheduleAtFixedRate(delay, rate) (() => {
      searchTaskCreator ! CreateForAllActive()
    }) (searchTaskCreator.executionContext)
    searchTaskCreator.log.info(s"scheduled searches with delay ${delay}s and interval ${rate}s")
  }
